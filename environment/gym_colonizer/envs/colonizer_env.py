...
class Bubble():
    def __init__(self, center_x=0, center_y=0, color=None):
        self.position = [center_x+200,center_y+200]
        BUBBLE_RADIUS = 20  # in pixel
        self.color = color
    ...
    def draw(self,pygame,screen, color=255, n=5, radius=100, position=[250,250]):
        pi2 = 2 * 3.14


        for i in range(0, n):
            pygame.draw.line(screen, color, self.position, (cos(i / n * pi2) * radius + self.position[0], sin(i / n * pi2) * radius + self.position[1]))

        pygame.draw.lines(screen, color, True, [(cos(i / n * pi2) * radius + self.position[0], sin(i / n * pi2) * radius + self.position[1]) for i in range(0, n)])

        #pygame.gfxdraw.filled_circle(
        #                        screen, round(
        #                            self.center_x), round(
        #                            self.center_y), BUBBLE_RADIUS, self.color)
...
class ColonizerEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self.window_height = (self.array_height * BUBBLE_RADIUS * 2
                              + self.spacing * (self.array_height + 2)
                              + 6 * BUBBLE_RADIUS)
        self.window_width = (self.array_width * BUBBLE_RADIUS * 2
                             + self.spacing * (self.array_width + 1)
                             + 5 + 0.5 * self.spacing)
        self.start_x = self.window_width / 2.0
        self.start_y = self.window_height - self.spacing - BUBBLE_RADIUS
                ...
                for row in range(self.array_height):
                    for column in range(self.array_width):
                        if self.last_board[row][column].color is not None:
                            ...
                            bubble.draw(pygame,self.screen)
                            #pygame.gfxdraw.filled_circle(
                            #    self.screen, round(
                            #        bubble.center_x), round(
                            #        bubble.center_y), BUBBLE_RADIUS, bubble.color)
                ...
                for position in self.last_positions:
                    if last_x is not None and last_y is not None:
                        pygame.gfxdraw.filled_circle(
                            self.screen, round(
                            last_x), round(
                            last_y), BUBBLE_RADIUS, (255,255,255))
                    last_x, last_y = position[0], position[1]
                    pygame.gfxdraw.filled_circle(
                            self.screen, round(
                            position[0]), round(
                            position[1]), BUBBLE_RADIUS, self.last_color)
                    pygame.display.update()
                ...
                for row in range(self.array_height):
                    for column in range(self.array_width):
                        if self.board[row][column].color is not None:
                            ...
                            pygame.gfxdraw.filled_circle(
                                self.screen, round(
                                    bubble.position[0]), round(
                                    bubble.position[1]), BUBBLE_RADIUS, bubble.color)
                pygame.display.update()
        else:
    ...
    def step(self, action):
        ...
        while True:
            ...
            self.last_positions.append((self.next_bubble.position[0], self.next_bubble.position[1]))
        ...
        for row in range(self.array_height):
            for column in range(self.array_width):
                self.board[row][column].center_x = (
                    BUBBLE_RADIUS * 2 + self.spacing) * column + BUBBLE_RADIUS + self.spacing

        # adjust the x-value in every second row
        for row in range(1, self.array_height, 2):
            for column in range(self.array_width):
                self.board[row][column].center_x += BUBBLE_RADIUS + \
                    0.5 * self.spacing

        # calculate the row distance on the y-axis based on the spacing
        y_distance = abs(math.sqrt((2 * BUBBLE_RADIUS + self.spacing)
                                   ** 2 - (BUBBLE_RADIUS + 0.5 * self.spacing)**2))

        # set the y-values for every bubble
        for row in range(self.array_height):
            for column in range(self.array_width):
                self.board[row][column].position[1] = self.spacing + \
                    BUBBLE_RADIUS + row * y_distance
    ...
    def _move_next_bubble(self, angle):
        ...
        self.next_bubble.position[0] += xmove
        self.next_bubble.position[1] += ymove

        # collision with left wall
        if self.next_bubble.position[0] - BUBBLE_RADIUS <= self.spacing:
            angle = 180 - angle
        # collision with right wall
        elif self.next_bubble.position[0] + BUBBLE_RADIUS >= self.window_width - self.spacing:
            angle = 180 - angle
        return angle
            ...
            for column in range(self.array_width):
                if self.board[row][column].color is None:
                    ...
                    empty_bubble_list.append(
                        (distance, current.position[0], current.position[1], row, column))
    ...
    def _is_collided(self):
        ...
        """
        # collsion with top
        if self.next_bubble.position[1] - BUBBLE_RADIUS <= self.spacing:
            return True
        ...
        for row in range(self.array_height):
            for column in range(self.array_width):
                if self.board[row][column].color is None:
                    continue
                distance = self._bubble_center_distance(
                    self.next_bubble, self.board[row][column]) - BUBBLE_RADIUS * 2
                if distance < 0:
                    return True
        return False
    ...
    def _bubble_center_distance(self, bubble1, bubble2):
        ...
        """
        return math.sqrt((bubble1.position[0] - bubble2.position[0])
                         ** 2 + (bubble1.position[1] - bubble2.position[1])**2)
